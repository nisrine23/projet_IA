...
def display_department_info(department_name):
    if department_name == "🏭 Département de Production":
        ...
        """)
# Display department content based on the selection
if page == "🏠 Home":
    ...
    col1.metric("Active Alerts", "5 🔴", "+2 since yesterday")
    col2.metric("Operational Efficiency", "85%", "-3% since last month")
    col3.metric("Total Sections Monitored", "3 Departments")

elif page == "🆔 Identifiant":
    identifiant = st.text_input("Enter your ID", help="Please enter your user ID.")
    if identifiant:
        st.success(f"User ID: {identifiant}")

elif page == "⚙️ Département":
    st.markdown(f"### Département sélectionné: {department}")
